...
export const updateAnnotation = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  const { recordId, annotationId, text } = parsed.data;
  ...
  await updateAnnotationInDb(recordId, annotationId, {
    text,
